    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        ...
        try:
            async for chunk in llm_pipeline_stream(
                ...
                # If the pipeline yields a correction (formatter's cleaned output),
                # emit it as a separate 'correction' event so the frontend can replace
                # the raw streamed tokens with the properly formatted version.
                if chunk.startswith("__CORRECTION__:"):
                    correction = chunk[len("__CORRECTION__:"):]
                    full_answer = [correction]
                    safe_correction = correction.replace("\n", "<|n|>")
                    yield f"event: correction\ndata: {safe_correction}\n\n"
                    continue
