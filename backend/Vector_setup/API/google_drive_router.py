...
from Vector_setup.services.excel_ingest_service import ingest_excel_by_sheet, EXCEL_EXTENSIONS
...
#     # 4) Extract text
#     # Extract + index for excel docs
#     if synthetic_filename.lower().endswith(EXCEL_EXTENSIONS):
#         # Excel: ingested sheet-by-sheet
#         ingested_sheets = await ingest_excel_by_sheet(
#             raw_bytes=raw_bytes,
...
#             store=store,
#         )
#         text = f"Excel file with sheets: {', '.join(ingested_sheets)}"
#     else:
...
async def ingest_drive_file(
    ...
    - Excel files are ingested sheet by sheet.
    """
    tenant_id = current_user.tenant_id
    ...
    # ── Resolve collection + enforce ACL ──────────────────────────────────
    collection = get_collection_for_user_or_403(
        db=db,
        current_user=current_user,
        tenant_id=tenant_id,
    ...
    if mime_type.startswith("application/vnd.google-apps"):
        ...
        synthetic_filename = original_name
        request = service.files().get_media(fileId=req.file_id)
    ...
    # ── 4) Build doc_id + metadata (shared by both paths) ─────────────────
    doc_id = str(uuid.uuid4())
    ...
    # ── 5) Extract + Index ────────────────────────────────────────────────
    if synthetic_filename.lower().endswith(EXCEL_EXTENSIONS):
        # Excel: ingested sheet by sheet — add_document called inside
        ingested_sheets = await ingest_excel_by_sheet(
            raw_bytes=raw_bytes,
            original_name=original_name,
            synthetic_filename=synthetic_filename,
        ...
        text = f"Excel file ingested with sheets: {', '.join(ingested_sheets)}"

    else:
        # PDF, DOCX, TXT, MD, CSV — single document
        text = extract_text_from_upload(synthetic_filename, raw_bytes)
        if not isinstance(text, str) or not text.strip():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    # ── 7) Mark file as ingested (both paths) ─────────────────────────────
    mark_file_ingested(
        db=db,
        tenant_id=tenant_id,
        drive_file_id=req.file_id,
